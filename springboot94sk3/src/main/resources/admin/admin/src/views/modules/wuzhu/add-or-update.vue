...
export default {
  data() {
	...
	shenfenzheng : false,
      ...
      rules: {
          wuzhuzhanghao: [
                { required: true, message: '屋主账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          wuzhuxingming: [
                { required: true, message: '屋主姓名不能为空', trigger: 'blur' },
          ],
          fangwumingcheng: [
          ],
          xingbie: [
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wuzhu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wuzhuCrossAddOrUpdateFlag = false;
                  this.parent.search();
