    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="评价类型" prop="pingjialeixing">
          <el-select v-model="ruleForm.pingjialeixing" placeholder="请选择评价类型">
            <el-option
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.zukezhanghao!=''&&json.zukezhanghao){
              		this.ruleForm.zukezhanghao = json.zukezhanghao
		}
		if(json.zukexingming!=''&&json.zukexingming){
              		this.ruleForm.zukexingming = json.zukexingming
		}
        } else {
          this.$message.error(data.msg);
        }
      });
            this.pingjialeixingOptions = "好评,中评,差评".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `zukepingjia/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zukepingjia/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zukepingjiaCrossAddOrUpdateFlag = false;
                  this.parent.search();
