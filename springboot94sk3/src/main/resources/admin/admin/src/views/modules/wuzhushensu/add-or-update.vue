<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="租客账号" prop="zukezhanghao">
          <el-input v-model="ruleForm.zukezhanghao" 
              placeholder="租客账号" clearable  :readonly="ro.zukezhanghao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="租客账号" prop="zukezhanghao">
              <el-input v-model="ruleForm.zukezhanghao" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="申诉状态" prop="shensuzhuangtai">
          <el-select v-model="ruleForm.shensuzhuangtai" placeholder="请选择申诉状态">
            <el-option
                v-for="(item,index) in shensuzhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="申诉状态" prop="shensuzhuangtai">
	      <el-input v-model="ruleForm.shensuzhuangtai"
                placeholder="申诉状态" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="申诉时间" prop="shensushijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.shensushijian" 
                type="date"
                placeholder="申诉时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.shensushijian" label="申诉时间" prop="shensushijian">
              <el-input v-model="ruleForm.shensushijian" 
                placeholder="申诉时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="申诉内容" prop="shensuneirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="申诉内容"
                  v-model="ruleForm.shensuneirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shensuneirong" label="申诉内容" prop="shensuneirong">
                    <span>{{ruleForm.shensuneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.shensushijian = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fangwumingcheng = true;
          ...
          if(o=='shensuneirong'){
            this.ruleForm.shensuneirong = obj[o];
	    this.ro.shensuneirong = true;
            continue;
          }
          if(o=='shensuzhuangtai'){
            this.ruleForm.shensuzhuangtai = obj[o];
	    this.ro.shensuzhuangtai = true;
            continue;
          }
          if(o=='shensushijian'){
            this.ruleForm.shensushijian = obj[o];
	    this.ro.shensushijian = true;
            continue;
          }
          if(o=='wuzhuzhanghao'){
      ...
      this.$http({
		...
		if(json.wuzhuxingming!=''&&json.wuzhuxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.shensuzhuangtaiOptions = "申诉中,已处理".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `wuzhushensu/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wuzhushensu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wuzhushensuCrossAddOrUpdateFlag = false;
                  this.parent.search();
